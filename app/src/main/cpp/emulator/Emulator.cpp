...
#ifdef __ANDROID__
	SDL_Window *Emulator::pre_window = nullptr;
	SDL_Renderer *Emulator::pre_renderer = nullptr;
#endif
	...
	Emulator::Emulator(std::map<std::string, std::string> &_argv_map, bool _paused) : paused(_paused), argv_map(_argv_map), chipset(*new Chipset(*this))
	{
		...
		if (pre_window) {
			...
			window = SDL_CreateWindow(
				...
				SDL_WINDOWPOS_UNDEFINED,
				width, height,
				SDL_WINDOW_SHOWN | (IsResizable() ? SDL_WINDOW_RESIZABLE : 0)
			);
			if (!window)
...
//			(SDL_WINDOW_RESIZABLE)
		...
		const std::string interface_path = GetModelFilePath(GetModelInfo("interface_image_path"));
		int img_w, img_h, img_ch;
		unsigned char *img_data = stbi_load(interface_path.c_str(), &img_w, &img_h, &img_ch, 4);
		if (!img_data)
			PANIC("stbi_load failed for %s\n", interface_path.c_str());
		SDL_Surface *loaded_surface = SDL_CreateRGBSurfaceWithFormatFrom(img_data, img_w, img_h, 32, img_w * 4, SDL_PIXELFORMAT_RGBA32);
		if (!loaded_surface)
			PANIC("SDL_CreateRGBSurfaceWithFormatFrom failed: %s\n", SDL_GetError());
...
#ifdef __ANDROID__
			event.button.x = (int)((event.button.x - viewport_x) * (float) interface_background.dest.w / viewport_w);
			event.button.y = (int)((event.button.y - viewport_y) * (float) interface_background.dest.h / viewport_h);
#else
...
#ifdef __ANDROID__
			event.motion.x = (int)((event.motion.x - viewport_x) * (float) interface_background.dest.w / viewport_w);
			event.motion.y = (int)((event.motion.y - viewport_y) * (float) interface_background.dest.h / viewport_h);
			event.motion.xrel = (int)(event.motion.xrel * (float) interface_background.dest.w / viewport_w);
			event.motion.yrel = (int)(event.motion.yrel * (float) interface_background.dest.h / viewport_h);
#else
...
#ifdef __ANDROID__
		// 复用持久帧纹理；主循环每帧从它完整重绘屏幕（清屏+拷贝+按钮），
		// 否则 EGL swapBuffers 后 back buffer 内容未定义，空 present 会翻出残留导致闪烁
		if (!frame_tex || frame_tex_w != interface_background.dest.w || frame_tex_h != interface_background.dest.h)
		{
			if (frame_tex)
				SDL_DestroyTexture(frame_tex);
			frame_tex = SDL_CreateTexture(renderer, format, SDL_TEXTUREACCESS_TARGET, interface_background.dest.w, interface_background.dest.h);
			frame_tex_w = interface_background.dest.w;
			frame_tex_h = interface_background.dest.h;
		}
		SDL_Texture* tx = frame_tex;
#else
...
#ifdef __ANDROID__
		// 不在 Frame() 里画屏幕；主循环每帧 RenderCopy(frame_tex) 保证完整画面
#else
...
#ifdef __ANDROID__
	void Emulator::RecalcViewport()
	{
		int ww, wh;
		SDL_GetWindowSize(window, &ww, &wh);
		float scale = (float)ww / interface_background.dest.w;
		float scale_h = (float)wh / interface_background.dest.h;
		if (scale_h < scale)
			scale = scale_h;
		viewport_w = (int)(interface_background.dest.w * scale);
		viewport_h = (int)(interface_background.dest.h * scale);
		viewport_x = (ww - viewport_w) / 2;
		viewport_y = (wh - viewport_h) / 2;
	}

	SDL_Texture *Emulator::GetFrameTexture()
	{
		return frame_tex;
	}

	SDL_Rect Emulator::GetViewport()
	{
		SDL_Rect r {viewport_x, viewport_y, viewport_w, viewport_h};
		return r;
	}
#endif
